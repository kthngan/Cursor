...
function render() {
  ...
  for (const day of DAYS) {
    for (const period of PERIODS) {
      ...
      const caregiverChip = document.createElement("div");
      caregiverChip.className = "chip caregiver";
      caregiverChip.textContent = slot.caregiver || "Caregiver";
      caregiverChip.contentEditable = "true";
      caregiverChip.spellcheck = false;
      caregiverChip.addEventListener("blur", () => {
        slot.caregiver = caregiverChip.textContent.trim();
        saveState();
      });
